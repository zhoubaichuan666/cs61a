...
def swine_align(player_score, opponent_score):
    ...
    if player_score * opponent_score == 0:
        return False
    sma, big = min(player_score, opponent_score), max(player_score, opponent_score)
    while big % sma:
        tmp = big % sma
        big = sma
        sma = tmp
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    def player0():
        return take_turn(strategy0(score0, score1), score1, dice)

    def player1():
        return take_turn(strategy1(score1, score0), score0, dice)
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1, last=last_score, high=running_high):
        score = score0
        if 1 == who:
            score = score1
        if score - last > high:
            ...
            # print('Player', who, 'has', score, 'point(s)')
        last = score
...
def make_averaged(original_function, trials_count=1000):
    ...
    # BEGIN PROBLEM 8
    "*** YOUR CODE HERE ***"
    def cnt_average(*args, need_cnt=trials_count):
        cnt = 0
        sum_cnt = 0
        while cnt < need_cnt:
            sum_cnt += original_function(*args)
            cnt += 1
        return sum_cnt/need_cnt
    return cnt_average
    # END PROBLEM 8
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    while num_rolls <= 10:
        ...
        num = average_dice(num_rolls, dice)
        if max_num < num:
            max_num = num
            max_rolls = num_rolls
        num_rolls += 1
